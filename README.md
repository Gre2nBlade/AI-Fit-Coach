...
Бэкенд уже слушает `0.0.0.0:8000` и `CORS=*` — отдельно ничего настраивать не нужно.
Бэкенд + туннель должны быть запущены, пока приложением пользуются.

### Свой сервер со статическим (белым) IP

Если есть сервер с постоянным внешним IP — туннель не нужен, впиши адрес в
`frontend/www/js/config.js` (`window.AFC_API_BASE`) напрямую:

```js
window.AFC_API_BASE = "http://203.0.113.10:8000";
```

По возможности используй HTTPS + домен (например через Nginx/Caddy + Let's Encrypt):

```js
window.AFC_API_BASE = "https://api.твойдомен.ru";
```

Чистый `http` уже разрешён в APK (`capacitor.config.json` → `cleartext: true`), но
`https` надёжнее и не блокируется браузерами. После правки адреса пересобери фронт:
`cd frontend && npx cap sync android`.
